...
export default function ChoseEvent({circuit_id}) {
      ...
      <ul>
        {events.map(event => (
          <li key={event.id}>{event.name}
            ...
            <button onClick={() => { 
              dispatch({ type: 'add_event', event_id: event.id, circuit_id: circuit_id })
            }}>
              Add to circuit
            </button>
          </li>
...
function eventsReducer(events, action) {
  switch (action.type) {
    ...
    case 'add_event': {
      axios.patch(`http://localhost:3000/circuits/${action.circuit_id}/add_event`,
        {
          circuit: {
